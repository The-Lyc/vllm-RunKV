# Layer-wise KV Cache Recompute + RunKV IO Hybrid（V1 仅 Llama / 单机单卡）
...
- **仅 Llama 系模型**：仅支持 `vllm/model_executor/models/llama.py` 中的 `LlamaForCausalLM` 及其 `LlamaDecoderLayer/LlamaAttention` 结构。
- **仅 decoder self-attention（Llama 结构）**：V1 只做“能工作且对齐代码事实”的实现，不做跨模型泛化、不做 MLA、不做 Q-recompute（对 Llama 的 MHA/GQA 都应保持正确性；收益大小不在 V1 讨论范围）。
...
- 在 **IO load 进行中（load_stream）** 时，在 default stream 启动 recompute（写 staging 中被 skip 的 blocks）。
- recompute 与 IO copy 在 GPU 上并行，达到真正 overlap。
...
3) **立即在 default stream 启动 recompute(K/V → reshape_and_cache_flash)**
4) `mapper.sync_load_layer(layer_idx)`（确保 IO blocks ready；该 sync 应尽可能被 recompute 时间掩盖）
5) prefetch 下一层：
...
对 LlamaDecoderLayer，attention 的输入是：

- `hidden_states_normed`：`LlamaDecoderLayer.input_layernorm(...)` 的输出 hidden states（注意 residual 存在时返回 tuple）

V1 选择只存这一份张量的原因：

- recompute 的目标是重建 **K/V**，在 LlamaAttention 中 K/V 仅依赖 `qkv_proj(hidden_states_normed)` 和 `rotary_emb(positions, q, k)`。
...
V1 只支持 Llama，因此可以通过模型结构定位每层：

- `llama_model = self.model`（类型：`LlamaForCausalLM`）
- `decoder_layers = llama_model.model.layers`（`LlamaModel.layers`）
- 每个 `decoder_layer` 是 `LlamaDecoderLayer`
- 对每层 `layer_idx`，给 `decoder_layer.input_layernorm` 注册 `forward_hook`
...
### 6.3 per-layer IO prefix mask（必须实现）

每层 `layer_idx` 的 prefix tokens：

- `prefix_tokens = io_prefix_blocks[layer_idx] * block_size`

hook 内仅保存满足以下条件的 scheduled tokens：

- `positions_np[token_i] >= prefix_tokens`

prefix 内 tokens 永远走 IO，不需要保存 hidden states（节省 CPU 内存与 D2H）。

### 6.4 hook 写入 CPU store 的精确算法（必须实现）

在 hook 内（伪代码，必须按语义实现；允许实现细节不同）：

1) 基于 `self._lr_positions_np` 构造 `mask`（bool numpy 或 torch CPU tensor）
2) 选择 token 子集：
   - `logical_ids = self._lr_logical_ids_np[mask]`
   - `offsets = self._lr_block_offsets_np[mask]`
   - `positions = self._lr_positions_np[mask]`
   - `hs = hidden_states_normed[mask_gpu]`（mask_gpu 对应 token 维度）
3) **D2H**：
...
- 在 `if self.use_runkv: self._sync_all_runkv_offloads()` 之后
- 若启用 layer recompute：
  - 调用 `LayerRecomputeManager.sync_hs_d2h()`：
...
- `__init__(...)`
  - 输入：`device`、`kv_offload_config`、`kv_cache_config`（用于 `num_cpu_blocks`）、`hidden_size`、`block_size`、`num_layers`、`model`（LlamaForCausalLM）等
...
- `register_layernorm_hooks(gpu_model_runner: GPUModelRunner) -> None`
  - 给每个 `LlamaDecoderLayer.input_layernorm` 注册 hook
...
- `recompute_kv_for_layer(*, layer_idx: int, layer_name: str, gid: int, mapper: PagedBlockMapper, attn_module: Attention, skip_block_ids: set[int]) -> None`
  ...
  - 使用已经计算好的 `skip_block_ids` 执行 7.5 的 recompute 写 staging（不再在内部重复计算 skip）
...
6) 修改 `GPUModelRunner._runkv_pre_hook()`：
   - 新时序：load_async(skip) → recompute → sync_load → prefetch_next(skip_next)
...
- 已新增：
  - `vllm/v1/worker/layer_recompute.py`
    ...
    - 已实现 recompute 路径（Llama `qkv_proj + rotary_emb + reshape_and_cache_flash`）
- 已新增测试：
  - `tests/v1/kv_offload/test_layer_recompute_manager.py`
    - `test_begin_step_resets_owner_changed_blocks`
    - `test_layernorm_hook_and_sync_store_suffix_tokens`
